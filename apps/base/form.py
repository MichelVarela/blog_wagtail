...
class AdminUserCreate(forms.ModelForm):
    ...
    def clean_email(self):
        ...
        new_email = Users.objects.filter(email=email)
        if new_email.count():
    ...
    def clean_password1(self):
        ...
        if not mat:
            raise forms.ValidationError(
                "The password does not comply with the requested format.")
        return password1
...
class UserRegisterForm(forms.ModelForm):
    ...
    def email_clean(self):
        email = self.cleaned_data["email"].lower()
        new_email = models.Users.objects.filter(email=email)
        if new_email.count():
